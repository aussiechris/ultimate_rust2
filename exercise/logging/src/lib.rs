...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!(target: "Frog::hop", "The frog just hopped. It has {} energy.", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target: "Frog::hop","The frog has run out of energy and is going to sleep .zZ .zZ");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!(target: "Frog::sleep", "Non fatal error: The frog is already asleep .zZ .zZ");
        } else {
            self.sleeping = true;
        }
    }
