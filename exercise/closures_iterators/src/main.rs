...
fn main() {
    ...
    // pairs
    //     .into_iter()
    //     .map(|(first, second)| (first + 1, second))
    //     .for_each(|pair| println!("{:?}", pair));

    // for loop style
    for (first, second) in pairs {
        println!("{:?}", (first + 1, second));
    }
    ...
    // functional style
    //TODO find a way to do this functionally without creating a new vector - is modifying old vector in place more efficient?
    let mod_numbers = numbers.into_iter().map(|x| x * 3).collect::<Vec<_>>();
    println!("{:?}", mod_numbers); // should print [3, 6, 9, 12]
